...
/* -----------------------
   ...
   ----------------------- */
VP_API void GetFrameInfo(VideoFrame* frame, VideoFrameInfo* out_info) {
    if (frame && out_info) {
...
VP_API void GetFrameData(VideoFrame* frame, uint8_t* dist_data) {
    if (frame) {
...
VideoPlayerErrorCode processDecodedVideoFrame(VideoPlayer* player, AVFrame* frame) {
    ...
    if (player->Context == nullptr || player->Context->videoStreamIdx < 0 || player->VideoBufferSize <= 0) {
    ...
    while (IsRunning.load())
    {
        ...
        if (packet->stream_index == videoIndex)
        {
            ...
            while (avcodec_receive_frame(codecCtx, frame) == 0)
            {
                ...
                CurrentTimeMills.store((int64_t)(pts_sec * 1000.0));
...
VP_API void DestroyVideoPlayer(VideoPlayer* player) {
    if (player) {
        // ensure thread stopped and resources released
...

        {
            ...
            if (player->AvFrame) {
                av_frame_free(&player->AvFrame);
                player->AvFrame = nullptr;
            }
...
VP_API bool Open(VideoPlayer* player, const char* file, VideoPlayerOptions options)
{
    ...
    // allocate and setup all resources under lock
    {
        ...
        player->Buffer.resize(kCustomIoBufferSize);
        ctx->ioContext = avio_alloc_context(
            player->Buffer.data(),
            (int)player->Buffer.size(),
            0,
            player,
            ReadCallback,
            nullptr,
        ...
        if (avformat_open_input(&ctx->avformatContext, NULL, NULL, NULL) != 0) {
            LogError("avformat_open_input failed");
            return false;
        }

        if (avformat_find_stream_info(ctx->avformatContext, NULL) < 0) {
        ...
        if (strncmp(file, "fd://", 5) == 0) {
            ...
            player->IO = std::make_unique<VideoFileStream>(file);
        }

        // open codecs - using your helper functions in FFmpegContext
        ctx->avformatContext = ctx->avformatContext; // keep as-is
        ...
        player->VideoBufferSize = 3 * player->Context->videoFrameSizeInBytes;
        player->VideoBuffer = av_mallocz(player->VideoBufferSize);
        ...
        if (player->Context->videoStreamIdx >= 0) {
            auto* st = player->Context->avformatContext->streams[player->Context->videoStreamIdx];
...
VP_API void Close(VideoPlayer* player)
{
    ...
    if (player->VideoBuffer) {
        av_freep(&player->VideoBuffer);
        player->VideoBuffer = nullptr;
    }

    if (player->AudioBuffer) {
        av_freep(&player->AudioBuffer);
        player->AudioBuffer = nullptr;
    }
